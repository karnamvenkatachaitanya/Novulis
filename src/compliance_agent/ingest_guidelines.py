...
def determine_url_path(section_name: str, content: str) -> str:
    """Determine the URL path for a guideline chunk based on section name and content.
    
    Priority: explicit URL paths > section name keywords > content keywords.
    Ordered from most specific to least specific to avoid misclassification.
    """
    combined = (section_name + " " + content).lower()
    section_lower = section_name.lower()
    
    # 1. Check for explicit URL paths in the text (highest priority)
    explicit_paths = [
        "/dashboard/settings", "/dashboard/my-applications", "/dashboard/facilities",
        "/dashboard/action-items", "/dashboard/user-management", "/dashboard/announcements",
        "/dashboard/faqs", "/dashboard/tickets", "/dashboard/contact",
        "/privacy", "/terms", "/login",
    ]
    ...
    # 2. Check section name (more reliable than content body)
    section_keyword_map = [
        ...
        ("login", "/login"),
        ("credentials", "/login"),
        ("workspace", "/dashboard/my-applications"),
        ("new waiver", "/dashboard/my-applications"),
        ("sidebar", "/shared"),
        ("navigation", "/shared"),
        ("header", "/shared"),
        ("footer", "/shared"),
    ...
    for keyword, path in section_keyword_map:
        if keyword in section_lower:
            return path
    
    # 3. Fall back to content keywords (least reliable)
    for keyword, path in section_keyword_map:
...
def is_probable_heading(line: str) -> bool:
    ...
    # Prevent metadata/address lines containing colons from being misclassified as headings
    if ":" in line:
        if not re.match(r'^(Figure|Table|SECTION|Appendix|Step)\b', line, re.IGNORECASE):
            return False

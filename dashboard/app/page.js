    ...
    <div className="min-h-screen p-6 flex flex-col justify-between">
      ...
      <header className="flex justify-between items-center mb-8 border-b border-gray-800 pb-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Real-Time Autonomous QA Agent Controller</p>
        </div>
      ...
      <main className="flex-1 grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8">
        ...
        <section className="flex flex-col gap-6">
          ...
          <div className="glass-panel p-6 flex flex-col gap-4">
            <h2 className="text-lg font-semibold text-indigo-400">Agent Sweeper Configuration</h2>
            
            <div className="flex flex-col gap-2">
              <span className="text-xs font-mono text-gray-400">Target Scopes:</span>
              <div className="grid grid-cols-1 gap-2 bg-gray-900/60 p-3 rounded-lg border border-gray-800">
                {DEFAULT_PATHS.map((p) => (
          ...
          <div className="glass-panel p-4 flex-1 flex flex-col min-h-[300px]">
            <div className="flex justify-between items-center mb-2 border-b border-gray-800 pb-2">
              <h2 className="text-sm font-bold text-cyan-400 font-mono">Terminal Outputs</h2>
              <div className="flex items-center gap-2">
                <label className="text-[10px] font-mono text-gray-400 cursor-pointer flex items-center gap-1 select-none">
                  <input
                    type="checkbox"
                    checked={autoScroll}
                ...
                <span className="text-xs font-mono text-gray-500">| python stdout</span>
              </div>
            </div>
            <div 
            ...
            >
              {logs.length === 0 && <p className="text-gray-600 italic">Terminal awaiting agent launch...</p>}
              {logs.map((log, idx) => (
        ...
        <section className="xl:col-span-2 flex flex-col gap-6">
          ...
          <div className="glass-panel overflow-hidden flex flex-col relative min-h-[400px]">
            {/* Mac Browser Header */}
            <div className="bg-gray-900/90 px-4 py-2 flex items-center gap-3 border-b border-gray-800">
              <div className="flex gap-1.5">
                <span className="w-3 h-3 rounded-full bg-rose-500 inline-block"></span>
                <span className="w-3 h-3 rounded-full bg-amber-500 inline-block"></span>
                <span className="w-3 h-3 rounded-full bg-emerald-500 inline-block"></span>
              </div>
              <div className="flex-1 bg-gray-950/80 rounded-md py-1 px-4 border border-gray-800 text-xs text-gray-400 text-center font-mono select-none overflow-hidden text-ellipsis">
            ...
            <div className="flex-1 bg-[#10131E] flex flex-col items-center justify-center p-6 min-h-[350px] relative">
              {/* Scan Beam animation */}
              {status === "running" && simStep === "dashboard" && <div className="scan-overlay" />}

              {/* Login Simulator view */}
              {simStep !== "dashboard" && simStep !== "idle" && (
                <div className="w-full max-w-sm glass-panel p-6 bg-gray-950/80 border border-gray-800 flex flex-col gap-4">
                  <h3 className="text-sm font-bold text-center text-indigo-400 uppercase tracking-widest font-mono">WaiverPro Login</h3>
                  <div className="flex flex-col gap-1 text-xs">
                    ...
                    <input
                      ...
                      readOnly
                      value={simEmail}
                      className="bg-gray-900 border border-gray-700 rounded p-2 text-white font-mono typing-cursor"
                    />
                  </div>
                  <div className="flex flex-col gap-1 text-xs">
                  ...
                  <button
                    className={`text-xs py-2 rounded text-white font-semibold font-mono ${
                      simStep === "submitting" ? "bg-emerald-600 animate-pulse" : "bg-indigo-600"
                    }`}
                  >
              ...
              {simStep === "idle" && status !== "success" && (
                <div className="text-center p-8 max-w-md flex flex-col items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-indigo-500/10 flex items-center justify-center border border-indigo-500/30 text-indigo-400 text-xl font-bold font-mono">
                    P
                  </div>
                  <div>
                    <h3 className="text-md font-semibold text-gray-200">Playwright Frame Idle</h3>
              ...
              {simStep === "dashboard" && (
                <div className="w-full h-full flex flex-col items-center justify-center gap-4">
                  {activeScreenshot ? (
                    <div className="relative border border-gray-800 rounded-lg overflow-hidden max-w-full max-h-[350px]">
                      <img
              ...
              {status === "success" && simStep === "idle" && (
                <div className="text-center p-8 max-w-md flex flex-col items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-emerald-500/10 flex items-center justify-center border border-emerald-500/30 text-emerald-400 text-lg">
                    ✓
                  </div>
                  <h3 className="text-md font-semibold text-emerald-400">Sweep Completed</h3>
      ...
      {/* 3. Detailed Results Pane (RAG & Judgement findings) */}
      {latestReport && (
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="glass-panel p-6 flex flex-col gap-4">
            <div className="flex justify-between items-center border-b border-gray-800 pb-3">
              <h2 className="text-lg font-bold text-indigo-400">LLM Audited Findings</h2>
              <span className="text-xs font-mono text-gray-500">Run ID: {latestReport.runId}</span>
            </div>

            <div className="flex-1 overflow-y-auto max-h-[400px] flex flex-col gap-4">
              {latestReport.reports.map((report) => (
                <div key={report.target_path} className="border border-gray-800 rounded-lg p-4 bg-gray-900/40">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className={`text-[10px] uppercase font-mono font-bold px-2 py-0.5 rounded-full ${
                      report.findings.length > 0 ? "bg-rose-500/10 text-rose-400 border border-rose-500/20" : "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"
                    }`}>
                  ...
                  {report.findings.length === 0 ? (
                    <p className="text-xs text-gray-500 italic">No discrepancies found on this page.</p>
                  ) : (
                        ...
                        <tbody>
                          {report.findings.map((f, idx) => (
                            <tr key={idx} className="border-b border-gray-800/40 hover:bg-gray-800/20">
                              <td className="py-2 pr-2 font-mono text-[10px] text-gray-400 max-w-[100px] overflow-hidden text-ellipsis">
          ...
          <div className="glass-panel p-6 flex flex-col gap-4">
            <div className="flex justify-between items-center border-b border-gray-800 pb-3">
              <h2 className="text-lg font-bold text-indigo-400">PDF Report & Alerting</h2>
              <span className="text-xs font-mono text-gray-500">ReportLab Document</span>
            </div>

            {latestReport.pdfPath ? (
              <div className="flex-1 flex flex-col gap-4">
                <div className="border border-gray-800 rounded-lg overflow-hidden flex-1 h-[250px] bg-slate-900 relative">
                  <iframe
                ...
                <div className="flex flex-col gap-3 p-3 bg-gray-900/60 rounded-lg border border-gray-800">
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-gray-400 font-mono">Attachment: <code>{latestReport.pdfPath.split(/[\\/]/).pop()}</code></span>
                    <button
      ...
      {/* 4. Footer */}
      <footer className="text-center text-xs text-gray-500 border-t border-gray-800 pt-4">
